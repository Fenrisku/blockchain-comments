...
func CreateChannel(sdk *fabsdk.FabricSDK, info *InitInfo) error {
	...
	// clientContext := sdk.Context(fabsdk.WithUser(info.OrgAdmin), fabsdk.WithOrg(info.Org1Name),fabsdk.WithOrg(info.Org2Name))
	clientContext := sdk.Context(fabsdk.WithUser(info.OrgAdmin), fabsdk.WithOrg(info.Org1Name))
	...
	mspClient, err := mspclient.New(sdk.Context(), mspclient.WithOrg(info.Org1Name))
	if err != nil {
	...
	adminContext := sdk.Context(fabsdk.WithUser(info.OrgAdmin), fabsdk.WithOrg(info.Org2Name))

	// Org resource management client
	orgResMgmt, err := resmgmt.New(adminContext)

	err = orgResMgmt.JoinChannel(info.ChannelID, resmgmt.WithRetry(retry.DefaultResMgmtOpts), resmgmt.WithOrdererEndpoint(info.OrdererOrgName))

	info.OrgResMgmt2 = orgResMgmt
...
func InstallAndInstantiateCC(sdk *fabsdk.FabricSDK, info *InitInfo) (*channel.Client, error) {
	...
	_, err = info.OrgResMgmt2.InstallCC(installCCReq, resmgmt.WithRetry(retry.DefaultResMgmtOpts))
	if err != nil {
	...
	clientChannelContext := sdk.ChannelContext(info.ChannelID, fabsdk.WithUser(info.UserName), fabsdk.WithOrg(info.Org1Name), fabsdk.WithOrg(info.Org2Name))
	...
	// clientChannelContext := sdk.ChannelContext(info.ChannelID, fabsdk.WithUser(info.UserName), fabsdk.WithOrg(info.Org2Name))
	// // returns a Client instance. Channel client can query chaincode, execute chaincode and register/unregister for chaincode events on specific channel.
	// channelClient, err := channel.New(clientChannelContext)
	// if err != nil {
	// 	return nil, fmt.Errorf("创建应用通道2客户端失败: %v", err)
